...
for i in range(len(test_files)):
    ...
    if score_margin <= 0.90:
        ...
        '''
        # CHECK SAVE THE AUDIO
        wav_output_filename = f'{timestamp}.wav'
        wavefile = wave.open(wav_output_filename, 'wb')
        wavefile.setnchannels(1)
        wavefile.setsampwidth(2)
        wavefile.setframerate(16000)
        wavefile.writeframes(audio_binary.numpy())
        wavefile.close()
        '''
...
print(f'Accuracy = {accuracy/tot_instances * 100}%')
print('Big inferences: ', big_inferences)
print(f'Comunication Cost: {communication_cost / 2**20:.2f} MB')
