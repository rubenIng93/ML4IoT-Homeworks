...
def save_as_keras(model, version):
    saved_model_dir = f'./HW3/models/{version}'
...
def convert_as_tflite(path, ptq=True, print_size=True, compression=True):
    ...
    tflite_models_path = './HW3_Group5_tflite_models/'
...
class SignalGenerator:
    ...
    def preprocess_with_stft(self, file_path):
        ...
        #spectrogram = tf.image.resize(spectrogram, [24, 24])
...
if VERSION == 'little':
    ...
    # convert in tflite
    convert_as_tflite(keras_dir) # togliere il compression = False!!

    # test the model after ptq
    test_ds = test_ds.unbatch().batch(1)

    # retrieve the tflite and predict over the test set
    interpreter = tf.lite.Interpreter(model_path='./HW3_Group5_tflite_models/little.tflite')
...
# development version big
    ...
    tf.data.experimental.save(test_ds, './big_test_set') # SAVE THE TEST DATASET
